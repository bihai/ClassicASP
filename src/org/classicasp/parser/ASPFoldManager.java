 ...
 * @author mobrien
 */
public class ASPFoldManager implements FoldManager
{
    ...
    public void initFolds(FoldHierarchyTransaction transaction) 
    {
        ...
        TokenSequence<ASPTokenId> ts = (TokenSequence<ASPTokenId>) hi.tokenSequence();
        FoldType type = null;
        int start = 0;
        int offset = 0;
        while (ts.moveNext()) 
        {
            offset = ts.offset();
            Token<ASPTokenId> token = ts.token();
            ASPTokenId id = token.id();
